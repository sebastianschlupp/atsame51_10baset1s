...
ADC_Status current_adc0_status = {ADC_POSINPUT_AIN15,ADC_NEGINPUT_GND,false};
/* ************************************************************************** */
/* ************************************************************************** */
...
/** 
  ...
  @Errors
    SYS_USER_ERROR_FLAGS as defined in userapp.h are set if needed
 */
void AIN_ADC0_ConversionTrigger(){
    ADC0_ChannelSelect(current_adc0_status.Pchannel, current_adc0_status.Nchannel);
    ADC0_ConversionStart();
    current_adc0_status.busy = true;
}
...
/** 
  ...
  @Returns
    -
  @Errors
    SYS_USER_ERROR_FLAGS as defined in userapps.h are set if needed
 */
bool ADC_Reading_Example(){
    //Reading two ADCs and outputting values to command line
    bool function_done = false;
    //AN2
    if((current_adc0_status.busy == false) & (current_adc0_status.Pchannel == ADC_POSINPUT_AIN15) & (current_adc0_status.Nchannel == ADC_NEGINPUT_GND)){
        AIN_ADC0_ConversionTrigger();
    }
    if((current_adc0_status.busy == true) & (current_adc0_status.Pchannel == ADC_POSINPUT_AIN15) & (current_adc0_status.Nchannel == ADC_NEGINPUT_GND)){
        if(ADC0_ConversionStatusGet() == true){
            ...
            current_adc0_status.Pchannel = ADC_POSINPUT_AIN2;
            current_adc0_status.Nchannel = ADC_NEGINPUT_GND;
            current_adc0_status.busy = false;
        }
    }
    //AN3
    if((current_adc0_status.busy == false) & (current_adc0_status.Pchannel == ADC_POSINPUT_AIN2) & (current_adc0_status.Nchannel == ADC_NEGINPUT_GND)){
        AIN_ADC0_ConversionTrigger();
    }
    if((current_adc0_status.busy == true) & (current_adc0_status.Pchannel == ADC_POSINPUT_AIN2) & (current_adc0_status.Nchannel == ADC_NEGINPUT_GND)){
        if(ADC0_ConversionStatusGet() == true){
            ...
            current_adc0_status.Pchannel = ADC_POSINPUT_AIN15;
            current_adc0_status.Nchannel = ADC_NEGINPUT_GND;
            current_adc0_status.busy = false;
            function_done = true;
        }
